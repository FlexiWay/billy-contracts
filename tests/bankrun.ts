...
const labelKeypairs = async (umi) => {
  ...
  amman.addr.addLabel("global", curveSdk.Billy.globalPda[0]);
  amman.addr.addLabel("eventAuthority", curveSdk.Billy.evtAuthPda[0]);
...
describe("billy-bonding", () => {
  ...
  it("is initialized", async () => {
    ...
    const global = await adminSdk.Billy.fetchGlobalData();
